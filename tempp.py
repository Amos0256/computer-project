...
import datetime
import time
import dump_netflow_month_by_day
import shutil
...
def dump_netflow(date):
    # cmd = ["python3", "./dump_netflow_month_by_day.py", date]
    # subprocess.run(cmd, check=True)
    print(f"date: {date}")
    dump_netflow_month_by_day.main(date)


def main():
    START_DATE_STRING = "20230301"
    END_DATE_STRING = "20230609"
