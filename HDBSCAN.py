...
if df_flow.shape[0] > 10000:
    df_flow = df_flow.sample(10000, random_state=42)
else:
    df_flow = df_flow
...
for i in range(0, max(hdb_labels) + 1):
    print(
        f"Cluster {i}: {len(hdb_labels[hdb_labels==i]) / len(hdb_labels) * 100:.02f}%"
    )
...
# HDBSCAN_folder = 'HDBSCAN6'
# if not os.path.isdir(f"{BASE_PATH}{HDBSCAN_folder}"):
#     os.mkdir(f"{BASE_PATH}{HDBSCAN_folder}")
# #create output_folder
# out_folder = f"{BASE_PATH}{HDBSCAN_folder}/same_ip_diff_behavior"
...
# csv_out_folder = f"{BASE_PATH}{HDBSCAN_folder}/cluster"
...
X_tsne = TSNE(
    n_components=2, perplexity=31, init="pca", random_state=42, verbose=0
